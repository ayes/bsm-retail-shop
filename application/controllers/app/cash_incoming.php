 ...
 */
class Cash_incoming extends CI_Controller {
    function __construct() {
        parent::__construct();
        $this->user_logged_in();
        $this->load->model('app/cash_incoming_model');
    }
    ...
    function index() {
        $data['content'] = 'app/object/cash/cash_incoming_view';
        $data['get_cash_incoming'] = $this->cash_incoming_model->get_cash_incoming();
        $this->load->view('app/template_view', $data);
    }
    
    function save() {
           
            $this->cash_incoming_model->save();
            $this->session->set_flashdata('message', 'Cash incoming has been saved..');
            redirect('app/cash_incoming');
