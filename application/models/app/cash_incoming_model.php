 ...
 */
class Cash_incoming_model extends CI_Model {
    var $product_path;
    ...
    function get_cash_incoming() {
        $config['base_url'] = site_url('app/cash_incoming/index');
	$config['total_rows'] = $this->db->count_all('tbcash_incoming');
        $config['per_page'] = 30;
        $config['uri_segment'] = 4;
        $this->pagination->initialize($config);
        $this->db->limit(30, $this->uri->segment(4));
        $this->db->order_by('date', 'desc');
        $query = $this->db->get('tbcash_incoming');
        return $query;
    }
