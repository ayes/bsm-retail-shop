 ...
 */
class Cash_outgoing_model extends CI_Model {
    var $product_path;
    ...
    function get_cash_outgoing() {
        $config['base_url'] = site_url('app/cash_outgoing/index');
	$config['total_rows'] = $this->db->count_all('tbcash_outgoing');
        $config['per_page'] = 30;
        $config['uri_segment'] = 4;
        $this->pagination->initialize($config);
        $this->db->limit(30, $this->uri->segment(4));
        $this->db->order_by('date', 'desc');
        $query = $this->db->get('tbcash_outgoing');
        return $query;
    }
    ...
    function save() {
        ...
        $description = $this->input->post('description');
            ...
            $this->db->insert('tbcash_outgoing',$data);
